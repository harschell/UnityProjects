...
public class Sensor2 : MonoBehaviour
{
	...
	public GameObject light_cylinder_from_s1;
...
//				Debug.Log(active);
				if (!active)
				{
					if (toActivate!=null)
					{
						toActivate.GetComponent<Level1_activation_sensor2>().activate();
					}
					GetComponent<Level1_activation_sensor2>().activate();
					active = true;
				}
				return;
			}
		}
		if (light_cylinder_from_s1.renderer.enabled)
		{
			if (!active)
			{
				if (toActivate != null)
...
//			toActivate.GetComponent<Activation> ().deactivate ();
			GetComponent<Level1_activation_sensor2> ().deactivate ();
		}
		active = false;
	}
}