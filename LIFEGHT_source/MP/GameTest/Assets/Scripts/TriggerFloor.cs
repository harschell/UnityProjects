...
public class TriggerFloor : MonoBehaviour {
	...
	void Update () {
		//Debug.Log ("pos:" + GetComponent<PlayerLogic>().light.transform.position);
		//Debug.Log ("radious:" + GetComponent<PlayerLogic>().light.range);
		switch (typeTrigger) {//GetComponent<PlayerLogic>().light.transform.position
		case States.EQUAL:
